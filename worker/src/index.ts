...
function runStatus(succeeded: number, failed: number): IndexRunRecord['status'] {
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (path === '/index-file' && request.method === 'POST') {
        ...
        // Content-addressed run IDs make retries safe while allowing a changed file
        // to create a new auditable run. Vector IDs are deterministic as well.
        ...
        for (let i = 0; i < chunks.length; i += 100) {
          ...
          try {
            ...
            const message = error instanceof Error ? error.message : 'Vectorize upsert failed';
            for (const item of pending) {
              item.status = 'failed';
              item.retryable = true;
      ...
      // Queryable durable accounting for a previous index attempt.
