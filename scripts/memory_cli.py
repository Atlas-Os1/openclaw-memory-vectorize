...
def _index_file(agent: str, file: str) -> tuple[bool, int, str]:
    ...
    if result["status"] >= 400 or body.get("failed", 0) > 0:
        ...
        return False, body.get("succeeded", 0), detail
...
def _index_one_agent(agent: str, days: int) -> dict:
    ...
    for i in range(days):
        ...
        # Missing daily files are expected; every other requested-file failure
        # remains visible and makes the command fail non-zero.
